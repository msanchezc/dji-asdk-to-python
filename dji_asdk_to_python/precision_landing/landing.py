...
class ArucoLanding:
    ...
    def __init__(self, drone_ip, camera_matrix, camera_distortion, marker_id, marker_size_cm):
        ...
        self.ast = ArucoSingleTracker(
            camera_distortion=camera_distortion, camera_matrix=camera_matrix)
    ...
    def start(self):
        ...
        while True:
            ...
            if marker_found:
                ...
                if z_marker < 120 and abs(yaw_camera) < 15 and abs(x_camera) < 8 and abs(y_camera - 10) < 8:
                    fc.startLanding()
                    print("LANDING")
                    self.rtp_manager.stopStream()

                print("pitch %s, roll %s, yaw %s, throttle %s" % (
                    fcd.getPitch(), fcd.getRoll(), fcd.getYaw(), fcd.getVerticalThrottle()))
